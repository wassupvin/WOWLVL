...
// ===== XP TABLE =====
const totalXP = { /* (tetap sama, ga gue ubah) */ 
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(opt =>
      opt.setName("user").setDescription("Nama user").setRequired(true))
    .addStringOption(opt =>
      opt.setName("level").setDescription("Contoh: 10 → 50").setRequired(true))
    .addStringOption(opt =>
      opt.setName("message").setDescription("Pesan testimonial").setRequired(true))
    .addAttachmentOption(opt =>
      opt.setName("image").setDescription("Screenshot bukti").setRequired(true))
];
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    // ===== (SISA CODING LU TETAP, GA DIUBAH) =====
